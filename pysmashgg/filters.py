...
def bracket_show_sets_filter(response):
    ...
    for node in response['data']['phaseGroup']['sets']['nodes']:
        cur_set = {}
        cur_set['id'] = node['id']
        entrant_1 = node['slots'][0]['entrant']
        entrant_2 = node['slots'][1]['entrant']
        if entrant_1:
            cur_set['entrant1Id'] = entrant_1['id']
            cur_set['entrant1Name'] = entrant_1['name']
        else:
            cur_set['entrant1Id'] = None
            cur_set['entrant1Name'] = None
        if entrant_2:
            cur_set['entrant2Id'] = entrant_2['id']
            cur_set['entrant2Name'] = entrant_2['name']
        else:
            cur_set['entrant2Id'] = None
            cur_set['entrant2Name'] = None
        ...
        if 'fullRoundText' in node.keys():
            cur_set['fullRoundText'] = node['fullRoundText']
        if 'identifier' in node.keys():
            cur_set['identifier'] = node['identifier']
        for j in range(0, 2):
            players = []
            entrant = node['slots'][j]['entrant']
            if entrant:
                for user in entrant['participants']:
                    cur_player = {}
                    cur_player['playerId'] = user['player']['id']
                    cur_player['playerTag'] = user['player']['gamerTag']
                    players.append(cur_player)
